...
def scrape_season_details(season: int) -> pd.DataFrame:
    ...
    """
    team_df, team_abrv = scrape_team_records(season)

    team_df = pd.merge(
        scrape_conf_standings(season, team_abrv), team_df, on="Team"
    )
    team_df.insert(1, "season", season)

    details_df = pd.merge(
        team_df, scrape_roster_accolades(season, team_abrv), on="Team"
    )
    details_df = pd.merge(
        details_df, scrape_playoff_records(season, team_abrv), on="Team"
    )
